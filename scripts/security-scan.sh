...
install_tools() {
  ...
  if ! check_tool gitleaks && [ "$CI_MODE" = false ]; then
    print_info "Gitleaks not found. Install from: https://github.com/gitleaks/gitleaks"
  fi
}
...
run_gitleaks() {
  ...
  if gitleaks detect --source="${PROJECT_ROOT}" --report-path="${output_file}" --no-git; then
