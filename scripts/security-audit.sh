...
if command_exists gosec; then
    ...
    print_warning "GoSec not installed. Run: go install example.com/securego/gosec/v2/cmd/gosec@latest"
fi
...
if command_exists nancy; then
    ...
    print_warning "Nancy not installed. Run: go install example.com/sonatype-nexus-community/nancy@latest"
fi
...
if command_exists trivy; then
    ...
    print_warning "Trivy not installed. Visit: https://aquasecurityhub.io/trivy/"
fi
...
if command_exists gitleaks; then
    ...
    print_warning "GitLeaks not installed. Visit: https://example.com/gitleaks/gitleaks"
fi
...
REPORT_FILE="security/audit-summary-$(date +%Y%m%d-%H%M%S).txt"
{
    ...
    echo "======================================"
    echo "Date: $(date)"
    echo " Commit: $( rev-parse HEAD 2>/dev/null || echo 'Not a  repository')"
    echo ""
    echo "Tools Run:"
