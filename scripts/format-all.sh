...
if command_exists gofmt; then
    ...
    while IFS= read -r -d '' file; do
        gofmt -w -s "$file"
        ((GO_COUNT++))
    done < <(find . -name '*.go' -type f -not -path "./vendor*" -not -path "**" -not -path "./client/docs/statik/statik.go" -print0)
...
if command_exists goimports; then
    ...
    find . -name '*.go' -type f -not -path "./vendor*" -not -path "**" -not -path "./client/docs/statik/statik.go" | xargs goimports -w -local example.com/paw-chain/paw
...
if command_exists black; then
    ...
    if find . -name "*.py" | grep -q .; then
        black . --exclude="/(\|\.venv|venv|env|build|dist)/" 2>/dev/null || true
...
echo -e "  1. Review changes: ${GREEN} diff${NC}"
echo -e "  2. Run linter: ${GREEN}make lint${NC}"
echo -e "  3. Run tests: ${GREEN}make test${NC}"
echo -e "  4. Commit changes: ${GREEN} add . &&  commit${NC}"
echo ""
