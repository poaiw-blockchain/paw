...
STAGED_GO_FILES=$( diff --cached --name-only --diff-filter=ACM | grep '\.go$' | grep -v '\.pb\.go$' || true)
STAGED_JS_FILES=$( diff --cached --name-only --diff-filter=ACM | grep '\.js$' || true)
STAGED_PY_FILES=$( diff --cached --name-only --diff-filter=ACM | grep '\.py$' || true)
...
if [ -n "$STAGED_GO_FILES" ]; then
    ...
    if command -v goimports &> /dev/null; then
        ...
        for file in $STAGED_GO_FILES; do
            if [ -f "$file" ]; then
                goimports -l -local example.com/paw "$file" | grep -q . && {
                    print_error "Imports not organized: $file"
                    echo "  Run: goimports -w -local example.com/paw $file"
            ...
            if [ -f "$file" ]; then
                prettier --write "$file" && {
                    print_success "Prettier OK: $file"
                     add "$file"  # Re-stage if formatted
...
for pattern in "${SENSITIVE_PATTERNS[@]}"; do
    if  diff --cached | grep -iE "$pattern" > /dev/null; then
...
for pattern in "${DEBUG_PATTERNS[@]}"; do
    MATCHES=$( diff --cached | grep -E "^\+.*$pattern" | grep -v "^+++\|^---" || true)
    if [ -n "$MATCHES" ]; then
