...
#
# This script installs Git hooks for the PAW blockchain project.
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   PAW Blockchain - Git Hooks Installation Script      ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════╝${NC}"
echo ""
...
check_git_repo() {
    if [ ! -d "${PROJECT_ROOT}/.git" ]; then
        print_error "Not a git repository. Please run this script from the PAW project root."
        exit 1
    fi
...
install_custom_hooks() {
    print_info "Installing custom Git hooks..."
...
main() {
    ...
    echo "  1. Test the hooks: git commit --allow-empty -m \"test: verify hooks\""
