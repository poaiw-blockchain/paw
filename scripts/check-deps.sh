...
if command_exists go-mod-outdated; then
    ...
    echo "Install with: go install example.com/psampaz/go-mod-outdated@latest"
fi
...
if command_exists go-licenses; then
    ...
    echo "Install with: go install example.com/google/go-licenses@latest"
fi
...
if go list -m all | grep -q "example.com/ugorji/go/codec"; then
    print_warning "Found example.com/ugorji/go/codec - consider alternatives"
    BAD_PACKAGES=1
fi
