...
if (-not (Test-CommandExists golangci-lint)) {
    ...
    go install example.com/golangci/golangci-lint/cmd/golangci-lint@v1.55.2
} else {
...
# Setup  hooks
Write-Host "`nSetting up  hooks..." -ForegroundColor Yellow
if (Test-Path "") {
    $hookPath = "\hooks"
...
'@
    ...
    Write-Host "! Not a  repository, skipping  hooks" -ForegroundColor Yellow
}
