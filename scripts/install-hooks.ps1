...
#
# This script installs Git hooks for the PAW blockchain project on Windows.
...
function Write-Header {
    ...
    Write-Host "║   PAW Blockchain - Git Hooks Installation Script      ║" -ForegroundColor $Colors.Blue
    Write-Host "╚════════════════════════════════════════════════════════╝" -ForegroundColor $Colors.Blue
    Write-Host ""
}
...
function Test-GitRepository {
    if (-not (Test-Path (Join-Path $ProjectRoot ".git"))) {
        Write-Error "Not a git repository. Please run this script from the PAW project root."
        exit 1
    }
...
function Install-CustomHooks {
    Write-Info "Installing custom Git hooks..."
...
function Main {
    ...
    Write-Host "  1. Test the hooks: git commit --allow-empty -m 'test: verify hooks'"
