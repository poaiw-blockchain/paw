...
scripts=(
  "scripts/testnet/runbook/k6_rpc_read.js"
  "scripts/testnet/runbook/k6_rest_read.js"
)

if [ -n "${GRAPHQL_URL:-}" ]; then
  scripts+=("scripts/testnet/runbook/k6_graphql_read.js")
fi

# WS uses RPC_URL by default; allow override via RPC_WS_URL
if [ -n "${RPC_URL:-}" ] || [ -n "${RPC_WS_URL:-}" ]; then
  scripts+=("scripts/testnet/runbook/k6_ws_subscribe.js")
fi
