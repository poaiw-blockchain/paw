...
const rest = __ENV.REST_URL;
if (!rest) {
  throw new Error('REST_URL is required');
}
...
export default function () {
  const latest = http.get(`${rest}/cosmos/base/tendermint/v1beta1/blocks/latest`);
  check(latest, { 'latest block 200': (r) => r.status === 200 });

  const params = http.get(`${rest}/cosmos/auth/v1beta1/params`);
  check(params, { 'auth params 200': (r) => r.status === 200 });
