...
if (-not $SkipGoSec) {
    ...
    if (Test-Command "gosec") {
        ...
        Write-Warning "GoSec not installed. Run: go install github.com/securego/gosec/v2/cmd/gosec@latest"
    }
}
...
if (-not $SkipNancy) {
    ...
    if (Test-Command "nancy") {
        ...
        Write-Warning "Nancy not installed. Run: go install github.com/sonatype-nexus-community/nancy@latest"
    }
}
...
if (-not $SkipTrivy) {
    ...
    if (Test-Command "trivy") {
        ...
        Write-Warning "Trivy not installed. Visit: https://aquasecurity.github.io/trivy/"
    }
}
...
if (-not $SkipGitLeaks) {
    ...
    if (Test-Command "gitleaks") {
        ...
        Write-Warning "GitLeaks not installed. Visit: https://github.com/gitleaks/gitleaks"
    }
}
...
$gitCommit = try { git rev-parse HEAD 2>$null } catch { "Not a git repository" }
