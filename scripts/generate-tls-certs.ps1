...
function Test-Dependencies {
    ...
    if (Test-Path "C:\Program Files\Git\usr\bin\openssl.exe") {
        $script:OpenSSL = "C:\Program Files\Git\usr\bin\openssl.exe"
        $version = & $script:OpenSSL version
        Write-Info "Found OpenSSL (Git): $version"
        return $true
    }
    ...
    Write-Host ""
    Write-Host "Alternatively, use Git Bash which includes OpenSSL:"
    Write-Host "  Run: ./scripts/generate-tls-certs.sh from Git Bash"
    exit 1
}
...
function New-GitIgnore {
    $gitignore = Join-Path $CertsDir ".gitignore"
...
"@
        ...
        Write-Info "Created .gitignore: $gitignore"
    }
}
