...
install-hooks-all:
	@echo "--> Installing all  hooks (pre-commit + husky)"
...
# Local test runner pinned to go1.23.1 with clean caches
test-local:
	@echo "--> Running local test suite with go1.23.1"
	@GOTOOLCHAIN=go1.23.1 go clean -cache -testcache
...
format:
	...
	@find . -name '*.go' -type f -not -path "./vendor*" -not -path "**" -not -path "./client/docs/statik/statik.go" | xargs gofmt -w -s
	@find . -name '*.go' -type f -not -path "./vendor*" -not -path "**" -not -path "./client/docs/statik/statik.go" | xargs misspell -w
	@find . -name '*.go' -type f -not -path "./vendor*" -not -path "**" -not -path "./client/docs/statik/statik.go" | xargs goimports -w -local github.com/paw-chain/paw
...
install-security-tools:
	...
	@echo "  - Trivy: https://aquasecurityhub.io/trivy/"
