...
const (
	RANDOMNESS_COMMITMENT_STATUS_UNSPECIFIED RandomnessCommitmentStatus = 0
	RANDOMNESS_COMMITMENT_STATUS_COMMITTED   RandomnessCommitmentStatus = 1
	RANDOMNESS_COMMITMENT_STATUS_REVEALED    RandomnessCommitmentStatus = 2
	RANDOMNESS_COMMITMENT_STATUS_EXPIRED     RandomnessCommitmentStatus = 3
)

var RandomnessCommitmentStatus_name = map[int32]string{
...
var RandomnessCommitmentStatus_value = map[string]int32{
...
func (x RandomnessCommitmentStatus) String() string {
	return proto.EnumName(RandomnessCommitmentStatus_name, int32(x))
}
...
// HashAlgorithm defines supported hash algorithms for verification proofs.
// SEC-3.4: Hash algorithm versioning for forward compatibility.
type HashAlgorithm int32

const (
	// HASH_ALGORITHM_UNSPECIFIED defaults to SHA256 for backwards compatibility
	HASH_ALGORITHM_UNSPECIFIED HashAlgorithm = 0
	// HASH_ALGORITHM_SHA256 is the default and currently only supported algorithm (version 1)
	HASH_ALGORITHM_SHA256 HashAlgorithm = 1
	// HASH_ALGORITHM_SHA3_256 reserved for future use (version 2)
	HASH_ALGORITHM_SHA3_256 HashAlgorithm = 2
	// HASH_ALGORITHM_BLAKE3 reserved for future use (version 3)
	HASH_ALGORITHM_BLAKE3 HashAlgorithm = 3
)

var HashAlgorithm_name = map[int32]string{
	0: "HASH_ALGORITHM_UNSPECIFIED",
	1: "HASH_ALGORITHM_SHA256",
	2: "HASH_ALGORITHM_SHA3_256",
...
type Params struct {
	...
	// reputation_decay_percent is the daily decay percentage for provider reputation scores
	// Expressed in basis points (100 = 1%, 500 = 5%, etc.)
	// Reputation decays towards neutral (0.5) by this percentage per day
	// Default: 100 basis points (1% per day)
	ReputationDecayPercent uint32 `protobuf:"varint,16,opt,name=reputation_decay_percent,json=reputationDecayPercent,proto3" json:"reputation_decay_percent,omitempty"`
	// enabled controls whether the Compute module is active.
	// When false, all compute operations (requests, providers, results) are disabled.
...
func (m *Params) GetReputationDecayPercent() uint32 {
	if m != nil {
		return m.ReputationDecayPercent
	}
	return 0
}
...
type GenesisState struct {
	...
	RandomnessCommitments []RandomnessCommitment `protobuf:"bytes,17,rep,name=randomness_commitments,json=randomnessCommitments,proto3" json:"randomness_commitments"`
...
func (m *GenesisState) GetRandomnessCommitments() []RandomnessCommitment {
	if m != nil {
		return m.RandomnessCommitments
	}
	return nil
}

func (m *GenesisState) GetAggregatedRandomness() []byte {
	if m != nil {
		return m.AggregatedRandomness
	}
	return nil
}
...
// SEC-3.4: Added hash_algorithm_version for algorithm agility.
type VerificationProof struct {
	// signature is the Ed25519 signature over the result hash
	Signature []byte `protobuf:"bytes,1,opt,name=signature,proto3" json:"signature,omitempty"`
	// public_key is the provider's public key for signature verification
	PublicKey []byte `protobuf:"bytes,2,opt,name=public_key,json=publicKey,proto3" json:"public_key,omitempty"`
	// merkle_root is the root hash of the execution trace merkle tree
	MerkleRoot []byte `protobuf:"bytes,3,opt,name=merkle_root,json=merkleRoot,proto3" json:"merkle_root,omitempty"`
	// merkle_proof is the merkle inclusion proof path
	MerkleProof [][]byte `protobuf:"bytes,4,rep,name=merkle_proof,json=merkleProof,proto3" json:"merkle_proof,omitempty"`
	// state_commitment is the commitment to final computation state
	StateCommitment []byte `protobuf:"bytes,5,opt,name=state_commitment,json=stateCommitment,proto3" json:"state_commitment,omitempty"`
	// execution_trace is the deterministic execution log hash
	ExecutionTrace []byte `protobuf:"bytes,6,opt,name=execution_trace,json=executionTrace,proto3" json:"execution_trace,omitempty"`
	// nonce is used for replay attack prevention
	Nonce uint64 `protobuf:"varint,7,opt,name=nonce,proto3" json:"nonce,omitempty"`
	// timestamp is the proof generation timestamp (Unix seconds)
	Timestamp int64 `protobuf:"varint,8,opt,name=timestamp,proto3" json:"timestamp,omitempty"`
	// hash_algorithm_version identifies which hash algorithm was used for this proof.
	// SEC-3.4: Enables forward compatibility when upgrading hash algorithms.
	// Version 0/1: SHA-256 (default, current algorithm)
	// Version 2: SHA3-256 (reserved for future use)
	// Version 3: BLAKE3 (reserved for future use)
	// Verifiers MUST reject proofs with unsupported algorithm versions.
	HashAlgorithmVersion uint32 `protobuf:"varint,9,opt,name=hash_algorithm_version,json=hashAlgorithmVersion,proto3" json:"hash_algorithm_version,omitempty"`
}

func (m *VerificationProof) Reset()         { *m = VerificationProof{} }
func (m *VerificationProof) String() string { return proto.CompactTextString(m) }
func (*VerificationProof) ProtoMessage()    {}
func (*VerificationProof) Descriptor() ([]byte, []int) {
	return fileDescriptor_8c2d79025873cb55, []int{23}
}
func (m *VerificationProof) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *VerificationProof) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_VerificationProof.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
...
func (m *VerificationProof) XXX_Merge(src proto.Message) {
	xxx_messageInfo_VerificationProof.Merge(m, src)
}
func (m *VerificationProof) XXX_Size() int {
	return m.Size()
}
func (m *VerificationProof) XXX_DiscardUnknown() {
	xxx_messageInfo_VerificationProof.DiscardUnknown(m)
}

var xxx_messageInfo_VerificationProof proto.InternalMessageInfo

func (m *VerificationProof) GetSignature() []byte {
	if m != nil {
		return m.Signature
	}
	return nil
...
func (m *VerificationProof) GetPublicKey() []byte {
	if m != nil {
		return m.PublicKey
	}
	return nil
...
func (m *VerificationProof) GetMerkleRoot() []byte {
	if m != nil {
		return m.MerkleRoot
	}
	return nil
...
func (m *VerificationProof) GetMerkleProof() [][]byte {
	if m != nil {
...
func (m *VerificationProof) GetStateCommitment() []byte {
	if m != nil {
...
func (m *VerificationProof) GetExecutionTrace() []byte {
	if m != nil {
...
func (m *VerificationProof) GetNonce() uint64 {
	if m != nil {
		return m.Nonce
	}
	return 0
...
func (m *VerificationProof) GetTimestamp() int64 {
	if m != nil {
		return m.Timestamp
	}
	return 0
...
func (m *VerificationProof) GetHashAlgorithmVersion() uint32 {
	if m != nil {
		return m.HashAlgorithmVersion
	}
	return 0
}
...
type RandomnessCommitment struct {
	...
	CommittedAt time.Time `protobuf:"bytes,6,opt,name=committed_at,json=committedAt,proto3,stdtime" json:"committed_at"`
	// revealed_at is when the value was revealed (if revealed)
	RevealedAt *time.Time `protobuf:"bytes,7,opt,name=revealed_at,json=revealedAt,proto3,stdtime" json:"revealed_at,omitempty"`
}

func (m *RandomnessCommitment) Reset()         { *m = RandomnessCommitment{} }
func (m *RandomnessCommitment) String() string { return proto.CompactTextString(m) }
func (*RandomnessCommitment) ProtoMessage()    {}
func (*RandomnessCommitment) Descriptor() ([]byte, []int) {
	return fileDescriptor_8c2d79025873cb55, []int{24}
}
func (m *RandomnessCommitment) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *RandomnessCommitment) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_RandomnessCommitment.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
...
func (m *RandomnessCommitment) XXX_Merge(src proto.Message) {
	xxx_messageInfo_RandomnessCommitment.Merge(m, src)
}
func (m *RandomnessCommitment) XXX_Size() int {
	return m.Size()
}
func (m *RandomnessCommitment) XXX_DiscardUnknown() {
	xxx_messageInfo_RandomnessCommitment.DiscardUnknown(m)
}

var xxx_messageInfo_RandomnessCommitment proto.InternalMessageInfo

func (m *RandomnessCommitment) GetValidator() string {
	if m != nil {
		return m.Validator
	}
	return ""
...
func (m *RandomnessCommitment) GetCommitmentHash() []byte {
	if m != nil {
...
func (m *RandomnessCommitment) GetRevealValue() []byte {
	if m != nil {
...
func (m *RandomnessCommitment) GetBlockHeight() int64 {
	if m != nil {
		return m.BlockHeight
	}
	return 0
...
func (m *RandomnessCommitment) GetStatus() RandomnessCommitmentStatus {
	if m != nil {
		return m.Status
	}
	return RANDOMNESS_COMMITMENT_STATUS_UNSPECIFIED
}

func (m *RandomnessCommitment) GetCommittedAt() time.Time {
	if m != nil {
...
func (m *RandomnessCommitment) GetRevealedAt() *time.Time {
	if m != nil {
		return m.RevealedAt
	}
	return nil
...
func init() {
	...
	proto.RegisterEnum("paw.compute.v1.RandomnessCommitmentStatus", RandomnessCommitmentStatus_name, RandomnessCommitmentStatus_value)
	proto.RegisterEnum("paw.compute.v1.HashAlgorithm", HashAlgorithm_name, HashAlgorithm_value)
	...
	proto.RegisterType((*VerificationProof)(nil), "paw.compute.v1.VerificationProof")
	proto.RegisterType((*RandomnessCommitment)(nil), "paw.compute.v1.RandomnessCommitment")
}
...
var fileDescriptor_8c2d79025873cb55 = []byte{
	...
	,
}
...
func (m *Params) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.ReputationDecayPercent != 0 {
		i = encodeVarintState(dAtA, i, uint64(m.ReputationDecayPercent))
		i--
		dAtA[i] = 0x1
		i--
		dAtA[i] = 0x80
...
func (m *GenesisState) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if len(m.AggregatedRandomness) > 0 {
		i -= len(m.AggregatedRandomness)
		copy(dAtA[i:], m.AggregatedRandomness)
		i = encodeVarintState(dAtA, i, uint64(len(m.AggregatedRandomness)))
		i--
		dAtA[i] = 0x1
		i--
		dAtA[i] = 0x92
	}
	if len(m.RandomnessCommitments) > 0 {
		for iNdEx := len(m.RandomnessCommitments) - 1; iNdEx >= 0; iNdEx-- {
			{
				size, err := m.RandomnessCommitments[iNdEx].MarshalToSizedBuffer(dAtA[:i])
				if err != nil {
					return 0, err
				}
				i -= size
...
func (m *VerificationProof) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *VerificationProof) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *VerificationProof) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.HashAlgorithmVersion != 0 {
		i = encodeVarintState(dAtA, i, uint64(m.HashAlgorithmVersion))
		i--
		dAtA[i] = 0x48
	}
	if m.Timestamp != 0 {
	...
	if len(m.ExecutionTrace) > 0 {
		i -= len(m.ExecutionTrace)
		copy(dAtA[i:], m.ExecutionTrace)
		i = encodeVarintState(dAtA, i, uint64(len(m.ExecutionTrace)))
		i--
		dAtA[i] = 0x32
	}
	if len(m.StateCommitment) > 0 {
		i -= len(m.StateCommitment)
		copy(dAtA[i:], m.StateCommitment)
		i = encodeVarintState(dAtA, i, uint64(len(m.StateCommitment)))
		i--
		dAtA[i] = 0x2a
	}
	if len(m.MerkleProof) > 0 {
		for iNdEx := len(m.MerkleProof) - 1; iNdEx >= 0; iNdEx-- {
			...
			i = encodeVarintState(dAtA, i, uint64(len(m.MerkleProof[iNdEx])))
			i--
			dAtA[i] = 0x22
		}
	}
...
func (m *RandomnessCommitment) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *RandomnessCommitment) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *RandomnessCommitment) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.RevealedAt != nil {
		n38, err38 := github_com_cosmos_gogoproto_types.StdTimeMarshalTo(*m.RevealedAt, dAtA[i-github_com_cosmos_gogoproto_types.SizeOfStdTime(*m.RevealedAt):])
		if err38 != nil {
			return 0, err38
		}
		i -= n38
		i = encodeVarintState(dAtA, i, uint64(n38))
		i--
		dAtA[i] = 0x3a
	}
	n39, err39 := github_com_cosmos_gogoproto_types.StdTimeMarshalTo(m.CommittedAt, dAtA[i-github_com_cosmos_gogoproto_types.SizeOfStdTime(m.CommittedAt):])
	if err39 != nil {
		return 0, err39
	}
	i -= n39
	i = encodeVarintState(dAtA, i, uint64(n39))
	i--
	dAtA[i] = 0x32
	if m.Status != 0 {
		i = encodeVarintState(dAtA, i, uint64(m.Status))
	...
	if len(m.RevealValue) > 0 {
		...
		i = encodeVarintState(dAtA, i, uint64(len(m.RevealValue)))
		i--
		dAtA[i] = 0x1a
	}
...
func (m *Params) Size() (n int) {
	...
	if m.ReputationDecayPercent != 0 {
		n += 2 + sovState(uint64(m.ReputationDecayPercent))
	}
	if m.Enabled {
		n += 3
	}
...
func (m *GenesisState) Size() (n int) {
	...
	if len(m.RandomnessCommitments) > 0 {
		for _, e := range m.RandomnessCommitments {
			l = e.Size()
			n += 2 + l + sovState(uint64(l))
		}
	}
	l = len(m.AggregatedRandomness)
	if l > 0 {
		n += 2 + l + sovState(uint64(l))
	}
	return n
...
func (m *VerificationProof) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	l = len(m.StateCommitment)
	if l > 0 {
		n += 1 + l + sovState(uint64(l))
	}
	l = len(m.ExecutionTrace)
	if l > 0 {
		n += 1 + l + sovState(uint64(l))
	}
	if m.Nonce != 0 {
	...
	if m.HashAlgorithmVersion != 0 {
		n += 1 + sovState(uint64(m.HashAlgorithmVersion))
	}
	return n
}

func (m *RandomnessCommitment) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	n += 1 + l + sovState(uint64(l))
	if m.RevealedAt != nil {
		l = github_com_cosmos_gogoproto_types.SizeOfStdTime(*m.RevealedAt)
		n += 1 + l + sovState(uint64(l))
	}
	return n
}
...
func (m *Params) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 16:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field ReputationDecayPercent", wireType)
			}
			m.ReputationDecayPercent = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowState
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.ReputationDecayPercent |= uint32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *GenesisState) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 17:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field RandomnessCommitments", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.RandomnessCommitments = append(m.RandomnessCommitments, RandomnessCommitment{})
			if err := m.RandomnessCommitments[len(m.RandomnessCommitments)-1].Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 18:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field AggregatedRandomness", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.AggregatedRandomness = append(m.AggregatedRandomness[:0], dAtA[iNdEx:postIndex]...)
			if m.AggregatedRandomness == nil {
				m.AggregatedRandomness = []byte{}
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
...
func (m *VerificationProof) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: VerificationProof: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: VerificationProof: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field StateCommitment", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.StateCommitment = append(m.StateCommitment[:0], dAtA[iNdEx:postIndex]...)
			if m.StateCommitment == nil {
				m.StateCommitment = []byte{}
			}
			iNdEx = postIndex
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ExecutionTrace", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.ExecutionTrace = append(m.ExecutionTrace[:0], dAtA[iNdEx:postIndex]...)
			if m.ExecutionTrace == nil {
				m.ExecutionTrace = []byte{}
			}
			iNdEx = postIndex
		case 7:
			if wireType != 0 {
		...
		case 9:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field HashAlgorithmVersion", wireType)
			}
			m.HashAlgorithmVersion = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowState
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.HashAlgorithmVersion |= uint32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *RandomnessCommitment) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: RandomnessCommitment: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: RandomnessCommitment: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 5:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Status |= RandomnessCommitmentStatus(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		...
		case 7:
			...
			if m.RevealedAt == nil {
				m.RevealedAt = new(time.Time)
			}
			if err := github_com_cosmos_gogoproto_types.StdTimeUnmarshal(m.RevealedAt, dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
