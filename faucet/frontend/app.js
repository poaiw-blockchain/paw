// PAW Testnet Faucet - Frontend Application
    ...
    try {
        ...
        container.innerHTML = state.recentTransactions.map(tx => `
            <div class="transaction-item">
                ...
                <div class="transaction-amount">+${formatAmount(tx.amount)} PAW</div>
            </div>
        `).join('');
    } catch (error) {
...
function validateAddress() {
    ...
    const addressPattern = /^paw1[a-z0-9]{38,58}$/;
    if (!addressPattern.test(address)) {
        errorElement.textContent = 'Invalid PAW address format. Address must start with "paw1"';
        errorElement.classList.add('show');
        return false;
    }
...
function showSuccess(data) {
    ...
    message.textContent = `Successfully sent ${formatAmount(data.amount)} PAW to ${truncateAddress(data.recipient)}`;

    if (data.tx_hash) {
        txLink.href = `https://explorer.paw-chain.com/tx/${data.tx_hash}`;
