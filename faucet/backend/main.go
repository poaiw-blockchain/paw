...
func main() {
	...
	// Initialize metrics with faucet info
	metrics.Initialize(Version, "testnet", cfg.ChainID, cfg.Denom, cfg.AmountPerRequest)

	// Start balance/node status monitoring goroutine
	go monitorFaucetStatus(cfg, faucetService, db)
	...
	// Prometheus metrics endpoint (exposed at root level for scraping)
...
// monitorFaucetStatus periodically updates Prometheus metrics for balance and node status
func monitorFaucetStatus(cfg *config.Config, svc *faucet.Service, db *database.DB) {
	ticker := time.NewTicker(30 * time.Second)
	defer ticker.Stop()

	// Run once immediately
	updateFaucetMetrics(cfg, svc, db)
...
func updateFaucetMetrics(cfg *config.Config, svc *faucet.Service, db *database.DB) {
	// Update balance
	...
	if db != nil {
		...
		metrics.UpdateDatabaseStatus(err == nil)
	}
}
