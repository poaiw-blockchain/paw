...
- **Dependency Review**: 's dependency analysis
...
```bash
# Linux
wget -qO - https://aquasecurityhub.io/trivy-repo/deb/public.key | sudo apt-key add -
echo "deb https://aquasecurityhub.io/trivy-repo/deb $(lsb_release -sc) main" | sudo tee -a /etc/apt/sources.list.d/trivy.list
sudo apt-get update
...
- Update `leaksignore` (use sparingly)
