...
func (sp *SyncProtocol) RequestState(peerID string, height int64) error {
	...
	// State sync is disabled in v1.0 - use block sync instead
	return errors.New("state sync is disabled in this version")
}
...
func (sp *SyncProtocol) performStateSync(peers []*PeerSyncStatus) error {
	sp.logger.Info("attempting state sync")

	// Check if state sync is configured
	if sp.getStateSnapshot == nil || sp.applyStateSnapshot == nil {
		sp.logger.Warn("state sync callbacks not configured, falling back to block sync")
		return errors.New("state sync not configured")
	}

	// NOTE: Full state sync implementation is in state_sync.go
	// This is a placeholder that attempts to use state snapshots if available

	// For now, we fall back to block sync as state sync requires
	// additional integration with the StateSyncProtocol
	sp.logger.Info("state sync protocol available but not integrated in this flow")
	return errors.New("state sync requires StateSyncProtocol - use block sync")
}
