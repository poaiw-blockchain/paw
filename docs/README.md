# PAW Documentation

Purpose
- Provide a concise, accurate reference for the current codebase and operations.

Core docs
- Whitepaper: `WHITEPAPER.md`
- Technical spec: `TECHNICAL_SPECIFICATION.md`
- P2P protocol notes: `p2p-protocol-spec.md` (prototype scope)

Architecture
- Overview: `architecture/PROJECT_ARCHITECTURE.md`

Guides
- CLI quick reference: `guides/CLI_QUICK_REFERENCE.md`
- Testing: `testing/QUICK_START.md`, `testing/TESTING_INFRASTRUCTURE.md`

Interoperability
- IBC architecture diagram: `ibc/ARCHITECTURE_DIAGRAM.md`
- Relayer config and security: see top-level `ibc/` directory

Security
- TLS certificates and guidance: `security/README.md`, `security/PRODUCTION_CERTIFICATES.md`
- Security testing recommendations: `SECURITY_TESTING_RECOMMENDATIONS.md`

Operations
- API reference (OpenAPI and Nginx sample): `api/`
- Upgrades: `upgrades/` (templates and process)
- Consensus artifacts: Genesis files must remain canonical (Comet/Cosmos JSON, ints-as-strings). No lenient parsing is allowed in code paths; operators must fix invalid genesis offline before running a node.

Proposals
- `proposals/SMART_CONTRACT_INTEGRATION_PROPOSAL.md`

Notes
- Some legacy and exploratory documents were archived under `archive/docs_legacy/` to keep this section lean.
